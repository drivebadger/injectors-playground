...
# This is an injector script. It will be run against all NFS network
...
#
#
# Exact matching NFS shares by UUID or serial number is not supported,
...
if [ -d "$base/klienci" ] && [ ! -f "$base/klienci/drivebadger.txt" ]; then
	echo "This is a test message from Drive Badger NFS generic injector." >"$base/klienci/drivebadger.txt"
fi
